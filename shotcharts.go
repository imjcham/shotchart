...
func processRequest(request *http.Request) (playerID int, player string) {

	if request.Form["names"] != nil {
		player = request.Form["names"][0]
		playerID = getPlayerID(player)
		fmt.Println("Player is", player)
		fmt.Println("Player ID is", getPlayerID(player))
	} else {
		playerID = 201935
		player = "blah"
	}
...
func getPlayerImage(playerID int) (url string) {
	url = "http://stats.nba.com/media/players/230x185/" + strconv.Itoa(playerID) + ".png"

	return url
}
func getPlayerID(player string) (playerID int) {

	players, _ := nbagame.API.Players.All()
	var m = make(map[string]int)
	for _, players := range players {
		playerName := players.FirstName + " " + players.LastName
		m[playerName] = players.ID
	}
	return m[player]
